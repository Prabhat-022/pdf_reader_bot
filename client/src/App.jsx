...
//             setChat([...newChat, { role: role, message: message }]);
...
//                     {/* Avatar */}
//                     <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${isUser ? 'bg-blue-500 ml-2' : isSystem ? 'bg-green-500 mr-2' : 'bg-purple-500 mr-2'
//                         }`}>
//                         {isUser ? (
...
const App = () => {
    ...
    // Auto scroll to bottom when new messages arrive (only when chat changes)
    useEffect(() => {
        chatEndRef.current?.scrollIntoView({ behavior: 'smooth' });
    }, [chat]);
    ...
    return (
        <div className="flex flex-col h-screen bg-gradient-to-br from-purple-50 to-blue-50">
            ...
            <div className="flex-1 overflow-hidden flex flex-col max-w-4xl mx-auto w-full">
                <div className="flex-1 overflow-y-auto px-6 py-4 space-y-1">
                    {chat.map((messages, index) => (
                        <MessageBubble key={`${index}-${messages.role}-${messages.message.slice(0, 20)}`} messages={messages} index={index} />
                    ))}
